...
class HandlerTest < Test::Unit::TestCase
  ...
  def test_esi_vars
    Net::HTTP.start("localhost", 9997) do |h|
      res = h.get("/basic_vars.html", { 'Cookie' => "i=test; path=/" } )
      assert_not_nil res
      assert_not_nil res.header
      assert_not_nil res.body
			assert_no_match /HTTP_COOKIE/, res.body
			assert_match /ui=test/, res.body
    end
  end
  
  def test_esi_remove
    Net::HTTP.start("localhost", 9997) do |h|
      res = h.get("/basic_remove.html")
      assert_not_nil res
      assert_not_nil res.header
      assert_not_nil res.body
			assert_no_match /www.example.com/, res.body
			assert_no_match /<a/, res.body
    end
  end
